...
struct ContentView: View {
	...
	func loadClient() async {
		...
		do {
			...
			let client = try Client.from(v1Bundle: keys, options: .init(api: .init(env: Constants.xmtpEnv, isSecure: false)))
			await MainActor.run {
				print("Connected")
	...
	func onTryDemo() {
		...
		Task {
			do {
				...
				let client = try await Client.create(account: account, options: .init(api: .init(env: Constants.xmtpEnv, isSecure: false)))
				let keys = client.v1keys
				try Keystore.saveKeys(address: client.address, keys: keys)
